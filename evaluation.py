...
def run_inception_jit(inputs, inception_model, num_batches=1, inceptionv3=False):
    ...
    return tfgan.eval.run_classifier_fn(inputs,
  ...
  """
    ...
    for i, tensor in enumerate(input_tensors):
        with tf.device(device_format.format(i)):
            ...
            res = run_inception_jit(tensor_on_device, inception_model, num_batches=num_batches, inceptionv3=inceptionv3)
    ...
    with tf.device('/CPU'):
        return {'pool_3': tf.concat(pool3, axis=0), 'logits': tf.concat(logits, axis=0) if not inceptionv3 else None}
